...
function initStyle(tagStyle) {
  tagStyle.a =
    'display:inline;color:#366092;word-break:break-all;' + (tagStyle.a || '')
  ...
  tagStyle.pre =
    'overflow:scroll;' +
    (tagStyle.pre || 'background-color:#f6f8fa;padding:5px;border-radius:5px;')
  tagStyle.s =
  ...
  switch (name) {
    ...
    case 'img':
      if (attrs.width) {
        attrs.style =
          'width:' +
          attrs.width +
          (/[0-9]/.test(attrs.width[attrs.width.length - 1]) ? 'px' : '') +
          ';' +
          attrs.style
        delete attrs.width
      }
    ...
    case 'audio':
      ...
      if (name == 'video') {
        ...
        if (attrs.height) {
          attrs.style =
            'height:' + parseFloat(attrs.height) + 'px;' + attrs.style
          delete attrs.height
        }
      }
