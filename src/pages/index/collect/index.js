 ...
 * @Last Modified time: 2019-06-11 21:30:29
 */
import classNames from 'classnames'
import Taro from '@tarojs/taro'
import { ScrollView, View, Text } from '@tarojs/components'
import { observer } from '@tarojs/mobx'
import { SectionTitle, Img } from '@components'
import './index.scss'
  ...
  return (
    <View className={classNames(cls, className)}>
      <View className='layout-wind'>
        <SectionTitle rightText='更多征集'>正在征集</SectionTitle>
      </View>
      <ScrollView className={`${cls}__scroll-view`} scrollX>
        {DS.map((item, index) => (
          ...
          >
            <Img width={250} height={168} src={item.image} />
            <View className={`${cls}__mask flex flex-justify-center`}>
              <Text className='t-plain t-24'>{item.title}</Text>
            </View>
          </View>
        ))}
      </ScrollView>
